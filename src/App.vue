  ...
  <main class="container" v-if="done && loggedin">
    ...
    <div class="column">
      <template v-for="(sv, idx) in userServers">
        <img @click="change_server(sv['serverID'])" v-bind:src="sv['img_url']" width='50px' height='50px'
          class='user-icon' style='margin-top: 0px;margin-top:10px;' />
      </template>
    </div>

    <div v-if="showSIDvar" class="login"
      style="display:flex;flex-direction:row; left:100px; top:50px; height:30px; width:660px; z-index:999999;">
      <i style="font-size: 12px"> {{ serverID }} </i>
    </div>
    <div class="chanels">
      <div class="server-header" @click="showSID()" style="border-bottom: 2px solid #141414; height: 60px;">
        <h3 style="margin-bottom: 0px;margin-top:5px"> {{userServers[userServers.findIndex(obj => obj.serverID ==
          serverID)].name}} </h3>
      </div>
      <template v-if="done">
        <template v-if="appState[appState.findIndex(obj => obj.serverID == serverID)].storedChannels !== undefined">
          <template v-for="(div, index) in appState[appState.findIndex(obj => obj.serverID == serverID)].storedChannels"
            :key="index">
            <template v-if="div['channelTag'] === textChannel">
              <button @click="change_channel(div['channelTag'])" class="channel_button"
                style="background-color: #a87678; color: white"># {{ div['channelTag'] }}</button>
            </template>
            <template v-else>
              <button @click="change_channel(div['channelTag'])" class="channel_button"># {{ div['channelTag']
              }}</button>
            </template>
          </template>
        </template>
      </template>
      <button @click="create_channel(serverID)" class="channel_button"
        style=" position: relative; text-align: center; padding-left: 0px; padding-right: 0px;"><b>+</b></button>

    </div>
    <div v-if="createChannelPopUp" class="login"
      style="display:flex;flex-direction:row; left:200px; top:100px; height:50px; width:280px">
    ...
    <div id="chat">
      <template v-if="appState[appState.findIndex(obj => obj.serverID == serverID)].serverUsers !== undefined">
        <template
          v-for="(div, index) in appState[appState.findIndex(obj => obj.serverID == serverID)].storedChannels[appState[appState.findIndex(obj => obj.serverID == serverID)].storedChannels.findIndex(obj => obj.channelTag == textChannel)]['messages']"
          :key="div">
          <div class="comp-mess">
            <img
              v-if="appState[appState.findIndex(obj => obj.serverID == serverID)].serverUsers[appState[appState.findIndex(obj => obj.serverID == serverID)].serverUsers.findIndex(obj => obj.username == div['username'])] !== undefined"
              v-bind:src="appState[appState.findIndex(obj => obj.serverID == serverID)].serverUsers[appState[appState.findIndex(obj => obj.serverID == serverID)].serverUsers.findIndex(obj => obj.username == div['username'])]['img_url']"
              width='40px' height='40px' class='user-icon' style='margin-top: 10px;margin-bottom:10px;' />
            <img v-else v-bind:src="'a'" width='40px' height='40px' class='user-icon'
            ...
            <div class="message">
              ...
              <div class="user" style="padding-top:6px;">
                ...
                <div class="mdate" style="font-size:12px;padding-left:5px;padding-top:1px;">
                  <i>{{ get_date(Number(div['datetime'])) }}</i>
                </div>
              </div>
              <div
                v-if="div['m_content'].startsWith('https:') && ['.jpg', '.png', '.jpeg', '.gif'].some(char => div['m_content'].endsWith(char))">
                <img v-bind:src="div['m_content']">
              </div>
              <div
                v-else-if="div['m_content'].startsWith('https:') && ['.ogg', '.mp4'].some(char => div['m_content'].endsWith(char))">
    ...
    <div class="server-users">
      <template v-for="(svuser, index) in appState[appState.findIndex(obj => obj.serverID == serverID)].serverUsers"
        :key="index">
