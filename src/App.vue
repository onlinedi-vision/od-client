...
this.info[this.info.findIndex(obj => obj.sid == msid)].divs[this.info[this.info.findIndex(obj => obj.sid == msid)].divs.findIndex(obj => obj.channelTag==mchn)]['messages'].unshift({"username":muser, "m_content":mess.join(' ')});
        ...
        invoke('getServers', {token: token, username: this.username})
          .then((res) => {
            ...
            for(let i = 0; i < servers['s_list'].length; i++) {
              ...
              invoke('getChannels', {host_url: 'https://onlinedi.vison', token: token, server: servers['s_list'][i], username: this.username})
              .then((res) => {
                let channels = JSON.parse(res)['c_list'];
                for(let j = 0; j<channels.length; j++) {
                  invoke('getMessages', {host_url: 'https://onlindi.vision', token: token, server:servers['s_list'][i], channel:channels[j]['channel_name'], username: this.username})
                    .then((res) => {
  ...
  methods: {
    addDiv(message) {
      ...
      let sname = this.name;
      this.name = '...';
      invoke('sendMessage', {host_url: 'https://onlinedi.vision', token:this.token, channel: this.textChannel, server: this.sid,  m_content: message, username:this.username }).then((res) => {
        console.log('taaa');
        this.name='';
      }).catch((err) =>{
      ...
      invoke('getServers', {token: this.token, username: this.username})
          .then((res) => {
            ...
            for(let i = 0; i < servers['s_list'].length; i++) {
              ...
              this.info.push({'sid': servers['s_list'][i], 'divs':[]});
              invoke('getChannels', {host_url: 'https://onlinedi.vision', token: this.token, server: servers['s_list'][i], username: this.username})
              .then((res) => {
                let channels = JSON.parse(res)['c_list'];
                for(let j = 0; j<channels.length; j++) {
                  invoke('getMessages', {host_url: 'https://onlinedi.vision', token: this.token, server:servers['s_list'][i], channel:channels[j]['channel_name'], username: this.username})
                    .then((res) => {
    ...
    get_messages(channel, server, token) {
      invoke('getMessages', {host_url: 'https://onlinedi.vision', token: token, server:server, channel:channel})
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
    ...
    createChannel() {
      invoke('createChannel', {host_url: 'https://onlinedi.vision', "username": this.username, "sid": this.sid, "token": this.token, "channel_name": this.nchn})
        .then((res) => {
    ...
    joinServer() {
      invoke('joinServer', {host_url: 'https://onlinedi.vision', "username": this.username, "token": this.token, "sid": this.joinsid})
        .then((res) => {
          this.token=JSON.parse(res)['token'];
          invoke('writeCredentials', {creds: JSON.stringify({'username': this.username, 'token': this.token }) })
            .then((res) => console.log(res))
